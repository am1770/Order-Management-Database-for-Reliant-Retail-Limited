...
-- i) IF customer creation date Year <2005 Then Category A ii) IF customer creation date Year >=2005 and <2011 Then Category B 
...
end),upper(customer_fname),' ',upper(customer_lname)) full_name,customer_email,customer_creation_date,
(case
...
-- i) IF Product Price > 200,00 then apply 20% discount ii) IF Product Price > 100,00 then apply 15% discount 
-- iii) IF Product Price =< 100,00 then apply 10% discount # Hint: Use CASE statement, no permanent change in table required. 
-- [NOTE: TABLES to be used - PRODUCT, ORDER_ITEMS TABLE]

select * from product order by product_id asc;
select * from order_items order by product_id asc;
...
from product p left join order_items oit on p.product_id = oit.product_id
...
-- [NOTE: TABLES to be used - PRODUCT_CLASS, PRODUCT]
...
-- 4. Write a query to display customer_id, full name, customer_email, customer_phone and country of customers who have
-- cancelled all the orders placed by them (USE SUB-QUERY)[NOTE: TABLES to be used - ONLINE_CUSTOMER, ADDRESSS,
-- OREDER_HEARDER]

select * from online_customer;
select * from address;
select * from order_header order by customer_id asc;

select cid.customer_id, concat(cid.customer_fname, "  ", cid.customer_lname) as Full_Name, cid.customer_email, cid.customer_phone, a.country
from online_customer cid, address a,
(select oh.customer_id, count(oh.order_id) from order_header oh
where oh.customer_id in (select oh1.customer_id from order_header oh1
                      where order_status='Cancelled')
group by customer_id
having count(customer_id) = 1) as a
where cid.ADDRESS_ID = a.ADDRESS_ID and cid.CUSTOMER_ID = a.CUSTOMER_ID;


-- 5. Write a query to display Shipper name, City to which it is catering, num of customer catered by the shipper in the city
-- and number of consignments delivered to that city for Shipper DHL [NOTE: TABLES to be used - SHIPPER,ONLINE_CUSTOMER,
-- ADDRESSS, ORDER_HEARDER]
...
s.SHIPPER_NAME,
a.CITY,
COUNT(oh.CUSTOMER_ID) 'Customer_catered',
COUNT(oh.ORDER_STATUS) 'Consignments_delivered'
FROM shipper s
...
-- 6. Write a query to display product_id, product_desc, product_quantity_avail, quantity sold and
-- show inventory Status of products as below as per below condition: a. For Electronics and Computer categories, if sales
-- till date is Zero then show 'No Sales in past, give discount to reduce inventory', if inventory quantity is less than 10%
-- of quantity sold,show 'Low inventory, need to add inventory', if inventory quantity is less than 50% of quantity sold,
-- show 'Medium inventory, need to add some inventory', if inventory quantity is more or equal to 50% of quantity sold,
-- show 'Sufficient inventory'.
-- b. For Mobiles and Watches categories, if sales till date is Zero then show 'No Sales in past,
-- give discount to reduce inventory', if inventory quantity is less than 20% of quantity sold, show 'Low inventory,
-- need to add inventory', if inventory quantity is less than 60% of quantity sold, show 'Medium inventory,
-- need to add some inventory', if inventory quantity is more or equal to 60% of quantity sold, show 'Sufficient inventory'
-- c. Rest of the categories, if sales till date is Zero then show 'No Sales in past, give discount to reduce inventory',
-- if inventory quantity is less than 30% of quantity sold, show 'Low inventory, need to add inventory', if inventory quantity is
-- less than 70% of quantity sold, show 'Medium inventory, need to add some inventory', if inventory quantity is more or equal
-- to 70% of quantity sold, show 'Sufficient inventory'-- (USE SUB-QUERY) [NOTE: TABLES to be used - PRODUCT, PRODUCT_CLASS, ORDER_HEADER]
...
select pc.PRODUCT_CLASS_DESC, p.PRODUCT_ID, p.PRODUCT_DESC, p.PRODUCT_QUANTITY_AVAIL,(p.product_price*OI.PRODUCT_QUANTITY) as sales,
SUM(OI.PRODUCT_QUANTITY) AS Quantity_sold,OH.ORDER_ID AS Order_ID,
    (case 
         when pc.PRODUCT_CLASS_DESC in ('Electronics','Computer') then
         (case when (p.product_price*OI.PRODUCT_QUANTITY) = 0 then 'No Sales in past, give discount to reduce inventory'
   ...
   INNER JOIN ORDER_ITEMS OH ON OI.ORDER_ID = OH.ORDER_ID
    inner join PRODUCT_CLASS pc on p.PRODUCT_CLASS_CODE = pc.PRODUCT_CLASS_CODE
    GROUP BY p.PRODUCT_ID, p.PRODUCT_DESC, p.PRODUCT_QUANTITY_AVAIL,OH.ORDER_ID;
...
select DISTINCT  oi.order_id,(p.len*p.width*p.height) AS VOLUME 
from order_items oi inner join  product p
on oi.product_id = p.product_id
where (p.len*p.width*p.height)<= (SELECT DISTINCT(C.LEN*C.WIDTH*C.HEIGHT) AS CAR_VOL 
...
select * from product;
 ...
 SELECT OC.CUSTOMER_ID,CONCAT(CUSTOMER_FNAME,' ',CUSTOMER_LNAME) AS Customer_FullName,OH.PAYMENT_MODE,
...
WHERE OH.ORDER_STATUS = 'Shipped' and OH.PAYMENT_MODE = 'Cash' and CUSTOMER_LNAME like 'G%' 
GROUP BY OH.ORDER_ID;
 
 
-- 9. Write a query to display product_id, product_desc and total quantity of products which are sold together with product id
-- 201 and are not shipped to city Bangalore and New Delhi. Display the output in descending order with respect to the tot_qty.
...
WHERE P.PRODUCT_ID = '201' AND OH.ORDER_STATUS ='Shipped' AND A.CITY NOT IN('Banglore','New Delhi') 
GROUP BY P.PRODUCT_CLASS_CODE,P.PRODUCT_DESC
ORDER BY Total_Quantity DESC;


-- 10. Write a query to display the order_id,customer_id and customer fullname, total quantity of products shipped for order
-- ids which are even and shipped to address where pincode is not starting with "5"
...
SELECT OC.CUSTOMER_ID, CONCAT(CUSTOMER_FNAME,' ',CUSTOMER_LNAME) AS Customer_FullName,OH.ORDER_ID,
SUM(OI.PRODUCT_QUANTITY) AS Total_Quantity
FROM ONLINE_CUSTOMER OC
INNER JOIN ORDER_HEADER OH ON OH.CUSTOMER_ID = OC.CUSTOMER_ID 
INNER JOIN ADDRESS A ON A.ADDRESS_ID = OC.ADDRESS_ID 
INNER JOIN ORDER_ITEMS OI ON OI.ORDER_ID = OH.ORDER_ID 
WHERE OH.ORDER_STATUS = 'Shipped' AND OH.ORDER_ID % 2 = 0 AND A.PINCODE NOT LIKE '5%'
GROUP BY OH.ORDER_ID
